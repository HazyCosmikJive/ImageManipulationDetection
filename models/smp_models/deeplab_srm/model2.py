...
from .srm import CombinedConv2D
...
class DeepLabV3Plus(SegmentationModel):
    ...
    def __init__(
        ...
        self.block1 = nn.Sequential(
            nn.Conv2d(in_channels=self.srm_encoder.out_channels[-1], out_channels=48, kernel_size=1, bias=False),
    ...
    def forward(self, x):
        ...
        # noise features through 4 layers resnet, size 32*32 now, upsample to 128*128 to concat
        srm_features = self.up(srm_features[-1])
        srm_features = self.block1(srm_features)
        ...
        # use scSE attention module after concat features
