...
class DeepLabV3Plus(SegmentationModel):
    ...
    def __init__(
        ...
        self.bayar = BayarConv2d(in_channels=3, out_channels=3, padding=2)
        self.srm = SRMConv2D()
        ...
        self.bayar_encoder = get_encoder(
            encoder_name,
            in_channels=in_channels,
            depth=encoder_depth,
            weights=encoder_weights
        ...
        self.attention = md.Attention('scse', in_channels=400)
    ...
    def forward(self, x):
        ...
        x_bayar = self.bayar(x)
        bayar_features = self.bayar_encoder(x_bayar)

        # print("** SRM FEATURE", srm_features[-4].shape)
        srm_features = self.block1(srm_features[-4])
        bayar_features = self.block1(bayar_features[-4])
        ...
        concat_features = torch.cat([aspp_features, hight_res_features, srm_features, bayar_features], dim=1)
