...
from smp_models.deeplabv3.decoder import DeepLabV3PlusDecoder, DeepLabV3Decoder
...
class DeepLabV3Plus(SegmentationModel):
    ...
    def __init__(
        ...
        self.srm = CombinedAttConv2D(in_channels=3)
        ...
        if aux_params is not None:
            ...
            self.classification_head = None
        print("Using model with noise head")

    def forward(self, x):
        """Sequentially pass `x` trough model`s encoder, decoder and heads"""
        srm_features = self.srm(x)
        features = self.encoder(srm_features)
