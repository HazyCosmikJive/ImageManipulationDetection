...
from .srm_att import CombinedConv2D as CombinedAttConv2D
...
class UnetPlusPlus(SegmentationModel):
    ...
    def __init__(
        ...
        # self.srm = CombinedConv2D(in_channels=in_channels)
        self.srm = CombinedAttConv2D(in_channels=in_channels)
    ...
    def forward(self, x):
        # pass `x` through model's srmpart, encoder, decoder and heads
        x = self.srm(x)  # x didn't change in srm by now, print test
